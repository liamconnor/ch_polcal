...
def _test_simulate_observation():
    ...
    noise = np.random.normal(0, 0.5, ntimes*2*2).reshape(-1, 2, 2)\
    ...
    return V.reshape(-1, 4)[:, (0,1,3)], ha_r, phase, J, P_U, P_Q
        
if __name__=='__main__':
    V, ha_r, ph, J, P_U, P_Q = _test_simulate_observation()
    ...
    f.create_dataset('P_U', data=P_U)
    f.create_dataset('P_Q', data=P_Q)
