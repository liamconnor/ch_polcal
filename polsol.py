...
def get_eigenvectors(Data, N):
	   ...
	   """
	   Gain_mat = np.zeros([n_ant, n_ant, N], np.complex128)
...
#                                Gain_mat[ant_j,ant_j,:] = 0 # Set autocorrelations to zero            
	   ...
	   gain_approx = np.zeros([n_ant, N, 2], np.complex128)
...
for freq in range(54,nfreq*0+55):
#    print freq
	QM = np.zeros([2, 2], np.complex128)

	V_Q_ = V_Q[:, freq]
	V_U_ = V_U[:, freq]
	V_Qmat = np.zeros([n_ant, n_ant], np.complex128)
	V_Umat = np.zeros([n_ant, n_ant], np.complex128)
